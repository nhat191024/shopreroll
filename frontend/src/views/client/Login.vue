<template>
    <div class="h-dvh relative z-10 flex flex-col items-center py-1">
        ...
        <Form @submit="onSubmit"
            class="w-11/12 md:w-3/5 xl:w-1/5 h-fit mt-2 bg-white border border-black border-1 rounded-xl flex flex-col gap-5 items-center">
            <p class="mt-6 mb-3 text-3xl md:text-4xl md:font-medium text-primary-50 text-shadow-custom">Đăng Nhập</p>
            <div>
                <FloatLabel>
                    <InputText v-model="username"/>
                    <label for="username">Tên đăng nhập</label>
                </FloatLabel>
                <p class="text-red-700 text-center mt-1">{{ usernameError }}</p>
            </div>
            <div class="mt-1 flex flex-col items-center">
                ...
                <p class="text-red-700 text-center mt-1">{{ passwordError }}</p>
            </div>
            ...
            <a href="" class="mb-3">Quên mật khẩu? Lấy lại</a>
        </Form>
    </div>
</template>
<script setup>
