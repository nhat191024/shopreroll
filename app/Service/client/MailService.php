...
class MailService
{
    ...
    public function sendForgotEmail($user, $resetPasswordLink)
    {
        try {
            Log::info('Sending password reset email to: ' . $user->email);
            $this->mail->addAddress($user->email);

            $subject = '=?UTF-8?B?' . base64_encode('Đặt lại mật khẩu tài khoản Shop Reroll') . '?=';

            $mailTemplate = '
            <div style="padding: 0 10px; margin-bottom: 25px">
                <div>
                    <p>Xin chào ' . $user->name . ',</p>
                    <p>Chúng tôi nhận được yêu cầu đặt lại mật khẩu cho tài khoản của bạn tại Shop Reroll.</p>
                    ...
                    <p>Nếu bạn gặp vấn đề với nút bên trên, vui lòng copy đường dẫn sau vào trình duyệt:</p>
                    <p style="word-break: break-all;">' . $resetPasswordLink . '</p>

                    <hr style="margin: 30px 0" />

                    <p style="color: #666;">Đây là email tự động, vui lòng không trả lời email này.</p>
                    <p style="margin-top: 30px;">Trân trọng,</p>
                    <p><strong>Đội ngũ Shop Reroll</strong></p>
                </div>
            </div>';
            ...
            // Clear all settings after sending
            $this->mail->clearAddresses();
            $this->mail->clearAttachments();
            $this->mail->ClearCCs();
            $this->mail->ClearBCCs();
            ...
            Log::info('Successfully sent password reset email to: ' . $user->email);
            return true;
        } catch (Exception $e) {
