                    ...
                    <form action="{{ route('admin.reroll_sub_category.edit') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="productDescription" aria-describedby=""
                                name="id_youtube" value="{{ old('id_youtube', $rerollSubCategoryInfo->id_youtube ?? '') }}"
                                placeholder="Nhập tên sản phẩm">
                        </div>
                        <div class="form-group">
                            <label for="">File download link</label>
                            <input type="text" class="form-control" id="productDescription" aria-describedby=""
                                name="file_download_link" value="{{ old('file_download_link', $rerollSubCategoryInfo->file_download_link ?? '') }}"
                                placeholder="Nhập tên sản phẩm">
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" id="categorySelect" name="reroll_category_id" required>
                                @foreach ($allRerollCategories as $id => $name)
                                    <option value="{{ $id }}" {{ old('reroll_category_id', $idRerollSubCategory) == $rerollSubCategoryInfo->reroll_category_id ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="image-frame" style="border: 2px dashed #ccc; padding: 10px; text-align: center; cursor: pointer;">
                            @if($rerollSubCategoryInfo->image)
                                <img src="{{ asset('image/thumb/' . $rerollSubCategoryInfo['image']) }}" alt="Current Image" id="currentImage" style="max-width: 150px; max-height: 150px;"/>
                            @else
